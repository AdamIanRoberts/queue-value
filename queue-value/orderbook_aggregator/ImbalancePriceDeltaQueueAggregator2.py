...
class ImbalancePriceDeltaQueueAggregator2(OrderbookAggregator):
    def __init__(
        self,
        date: datetime,
        ticker: str,
    ...
    def _build_raw_orderbook(self) -> pd.DataFrame:
        file_name = f"{self.ticker}_{str(self.date)}_orderbook_{self.levels}.csv"
    ...
    def queue_position_steps(self) -> list:
        return np.arange(
            self.queue_position_step_size,
            self.max_queue_position + self.queue_position_step_size,
            self.queue_position_step_size,
        )
    ...
    def orderbook_absorbing_states(self) -> list:
        return [
            (self.spread_size, x, y, z)  # Separate case where mid-price move == -spread_size
            for x in self.imbalance_steps
            for y in self.price_delta_steps
            for z in self.queue_position_steps
        ] + [
    ...
    def _process_raw_orderbook(self, raw_orderbook: pd.DataFrame) -> pd.DataFrame:
        drop_cols = [col for col in raw_orderbook.columns if col[-1] in [str(x) for x in range(2, self.levels)]]
        ...
        raw_orderbook["ask_spread"] = raw_orderbook["ask_price_1"] - raw_orderbook["ask_price_0"]
    ...
    def _discretise_orderbook(self):
        ...
        event_quantities["ask_price_1"] = orderbook.loc[event_quantities["time_bucket"].values]["ask_price_1"].values
        event_quantities = event_quantities[event_quantities["price"] == event_quantities["ask_price_1"]]
        event_quantities = (
        ...
        event_quantities = pd.DataFrame(
            ...
            columns=["limit_sell", "cancel_sell"],
            data=np.array(
                [
        ...
        orderbook["next_ask_spread"] = orderbook["ask_spread"].shift(-1).values
        ...
        orderbook = orderbook[orderbook["ask_spread"] == self.spread_size]  # Ignore states with ask spread > 1 tick
        orderbook = orderbook[orderbook["next_ask_spread"] == self.spread_size]
